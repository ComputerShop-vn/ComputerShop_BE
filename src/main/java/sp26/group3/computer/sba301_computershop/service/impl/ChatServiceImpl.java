...
public class ChatServiceImpl implements ChatService {
        ...
        public List<ConversationResponse> getConversations() {
                ...
                return conversationRepo.findAllByUserId(me.getUserId()).stream()
                                .map(conv -> {
                                        // Eager access within the stream — safe because findAllByUserId fetches the
                                        // full entity
                ...
                ChatConversation conv = conversationRepo.findByRoomKey(roomKey)
                                .orElseGet(() -> conversationRepo.save(
                                                ChatConversation.builder()
                                                                .roomKey(roomKey)
                                                                .user1(sender.getUserId() < receiver.getUserId()
                                                                                ? sender
                                                                                : receiver)
                                                                .user2(sender.getUserId() < receiver.getUserId()
                                                                                ? receiver
                                                                                : sender)
